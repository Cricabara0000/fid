...
```{r}
num_filas <- nrow(hepatitis)
num_columnas <- ncol(hepatitis)
# Recorremos todas las filas y columnas del DataFrame
  for (i in 1:num_filas) {
    for (j in 1:num_columnas) {
      valor <- hepatitis[i, j]  # Obtenemos el valor en la fila 'i' y columna 'j'
      # Realizamos la transformación: TRUE a 1 y FALSE a 0
      if (!is.na(valor) && hepatitis[i, j]=="True") {
        hepatitis[i, j] <- 1
      }else if(!is.na(valor) && hepatitis[i, j]=="False"){
        hepatitis[i, j] <- 0
      }
    }
  }
...
for (variable in variables) {
  ...
  tabla <- table(hepatitis[[variable]])
  cat("Recuento de 0s:", tabla[1], "\n")
  cat("Recuento de 1s:", tabla[2], "\n\n")
}
```
...
library(corrplot)
# Convertir todas las variables a tipo numérico
hepatitis <- sapply(hepatitis, function(x) as.numeric(as.character(x)))
...
En principio, podemos deducir que no están muy correlacionadas. Casi todas las variables se encuentran rondando valores menores que 0.5 excepto las variables 'fatigue'-'malaise' y 'malaise'-'anorexia'. Con respecto a la correlación con la variable dependiente (predictora), casi todas las variables presentan mala correlación, porque tienen un valor negativo, debajo de cero, de hecho, las variables 'age', 'fatigue', 'malaise', 'spleen-palpable', 'spiders', 'ascites', 'anorexia', 'varices', 'bilirubin' e 'histology' son los que peores datos presentan pues los valores se acercan a -0.5. Estas variables podrían ser eliminadas porque no muestran resultados significativos con la variable dependiente, además, tampoco muestran evidencias significativas de correlación entre el resto de las variables independientes, sin embargo, como las variables 'fatigue', 'malaise' y 'anorexia' presentan valores altos de correlación, no se deberían eliminar. Además, teniendo en cuenta la distribución de las variables, las variables 'spleen_palpable' y 'varices' están muy desbalanceadas. Recordando la distribución de las variables numéricas discretas y la correlación, en general, de las variables 'antivirals', 'anorexia', 'liver_big' con el resto de variables no presenta resultados significativos. Por ello, se decide eliminar las variables 'age','spleen-palpable', 'spiders', 'ascites', 'varices', 'bilirubin', 'antivirals', 'anorexia', 'liver_big' e 'histology'.

```{r}
# Seleccionar las variables que deseas mantener
hepatitis <- subset(hepatitis, select = -c(age, spleen_palpable, spiders, ascites, varices, bilirubin, antivirals, anorexia, liver_big, histology))
...
El conjunto de datos se reduce de 19 atributos a 9 atributos en su totalidad. Esta selección de atributos permitirá que los resultados del estudio sean más precisos y fiables.
