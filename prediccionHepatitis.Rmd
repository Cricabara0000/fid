...
```
Como podemos observar, si se agrupan en dos clusters, 98 irian al cluster 1 y 57 al cluster 2. En nuestro dataset hay 32 ejemplos correspondientes a "muere" y 123 correspondientes a "vive" y podríamos pensar que pueden estar medianamente bien diferenciados. Sin embargo, si observamos el valor de Between_SS / total_SS es de 19.99%, es un porcentaje muy bajo lo que indica que no hay una buena separación entre los clusters.


## Separación del conjunto de datos
```{r}
#para evaluar los modelos dividiremos nuestros datos en dos conjuntos
...
hepatitis_norm_split<-createDataPartition(hepatitis_norm$class, p=0.80, list = FALSE)
trainingSetNorm <- hepatitis_norm[hepatitis_norm_split,]
testSetNorm <- hepatitis_norm[-hepatitis_norm_split,]
...
modelo <- lm(sex ~ ., data = hepatitis_norm)
...
predictors <- hepatitis_norm[, -10]
target <- hepatitis_norm[, 10]  
...
# Se realiza en primer lugar la regresión lineal
# La variable dependiente será el sexo y las independientes el resto
modelo <- lm(sex ~ ., data = hepatitis_reduced_norm)
...
# Definiendo las variables predictoras y la variable objetivo
predictors <- hepatitis_reduced_norm[, -2]
target <- hepatitis_reduced_norm[, 2]  
...
En este punto evaluaremos diferentes modelos, dividiendo el conjunto mediante validación cruzada con k = 5 y en dos conjuntos (training-test) ,los modelos evaluados serán los siguientes: 
- Red neuronal
- Regresión logística
...
modelos_clasificacion <- list(
   "Red neuronal" = train(class ~ ., data=trainingSetNorm, method ="nnet",trControl=control),
  "Regresión Logística" = train(class ~ ., data = trainingSetNorm, method = "glm", trControl = control, family=binomial),
  "Máquinas de Vector de Soporte (SVM)" = train(class ~ ., data = trainingSetNorm, method = "svmRadial", trControl = control),
  "Random Forest" = train(class ~ ., data = trainingSetNorm, method = "rf", trControl = control)
)
...
for (nombre_modelo in names(modelos_clasificacion)) {
  ...
  predicciones <- predict(modelo, newdata = testSetNorm)
  #matriz de confusion 
  matrizConfusion <- confusionMatrix(predicciones, testSetNorm$class)
  ...
  especificidad <- matrizConfusion$byClass["Specificity"]
   ...
   roc_obj <- roc(testSetNorm$class,
                              as.numeric(predict(modelo, newdata = testSetNorm, type = "raw")))
    auc <- auc(roc_obj)
...
ajustes <- trainControl(method = 'cv', number = 5)
